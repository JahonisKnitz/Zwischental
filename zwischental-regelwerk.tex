...
\begin{enumerate}
\item \textbf{Gebäudekarten} --- 4 \textit{Jahreszeitstapel} aus Jahreszeitenkarten + 1 Sonderkarten (lila) zusammenstellen und verdeckt bereitlegen \textcolor{LumenGray400}{(siehe \textit{Jahreszeitstapel})}. Restliche Karten in Schachtel.
\item \textbf{Überfallkarten \& Würfel} --- Die 3 Überfall-Stapel (gelb, blau, rot) verdeckt in die Mitte legen. Den passenden Würfel unter jeden Stapel legen.
\end{enumerate}
...
\item \textbf{Zählscheibe \& Rathaus} --- Nimm dir eine zufällige Rathauskarte und eine Zählscheibe.
\end{enumerate}
...
\subsection*{Jahreszeitstapel vorbereiten}

Ein \textit{Jahreszeitstapel} besteht bei 2/3/4 Spielenden aus 12/18/24 Karten. Je nach Spielerzahl eine bestimmte Anzahl Jahreszeitkarten + Sonderkarten (je nach Jahreszeit - siehe Tabelle) zu einem Stapel zusammenmischen.

\begin{lumentable}{llll}
 \textbf{Pro Person:} & Jahreszeit & Sonder & Drafting-Richtung \\
...
\fbox{\parbox[c][\linewidth][c]{\linewidth}{\centering\small\color{LumenGray400}Grafik 8,6 × 4,3\,cm}}}%
\par{\fontsize{8.5}{11}\selectfont\color{LumenGray400}Anzahl Karten pro Person in Jahreszeitstapel}\par
...
\begin{lumentip}
Ist das Startfeld des Überfalls bekannt, lege deine Zählscheibe neben die entsprechende Karte als Markierung.
\end{lumentip}
...
Teile den \textit{Jahreszeitstapel} gleichmäßig auf Starthände aus je \textbf{6 Karten} auf. Die Person mit den wenigsten \lumenVP\textbf{} wählt zuerst eine Hand, dann weiter nach Punktestand, aufsteigend. Bei Gleichstand entscheidet die Einwohnerzahl der Siedlung (kleinere Zahl zuerst). Im Winter, verteile die Hände zufällig.
\par\vspace{4pt}
...
\item \textbf{Ersetzen} --- Karte auf \textit{belegtes} Feld legen \& alte weg.
...
Zähle die Punkte auf allen noch aktiven Gebäuden und auf allen \lumenPlundered\textbf{geplünderten} Karten mit Turm + 2 \lumenVP\textbf{} pro Münze auf Karten und notiere sie.
...
\lumeniconexplain{icon-season-herbst.png}{\textbf{Herbst} --- 4. Jahreszeit (Spielrunde). Letzte Wertung im Spiel.}

\chapter{Gebäudekarten}
...
& \lumencatalogcard{F19}{SO15 - H11 - F20 - SO16 - H12 - Laufrichtung festgelegt (gelber Würfel ignoriert) --- \textcolor{LumenGold}{\textbf{\textit{fragil}}}}\\
%%%Reihe 3
...
\lumenraidheading{LumenClayDark}{Überfallkarten}
...
& \lumenraidcard{C6}{kein Champion-Effekt}
...
\textbf{F: Champion C9 macht die Startkarte des Überfalls \textcolor{LumenGold}{\textbf{\textit{fragil}}}. Gilt das auch für Karten mit Turm/Luftschlösser (Z19, Z20)?}\\
...
Sie zieht weiter ohne Verluste.
...
Nein. Der Außenposten ignoriert das Baulimit vollständig.
